/**
 * Noob-Q specifics on top of the generic `@noob-audio-engineering/noob-vst-webgui-framework/vue` bridge: band
 ...
 *
 * Rules of use:
 * - `useNoobVstWebguiFramework()` may be called any time; it creates the client on first
...
import {
  ...
  useParam,
  useNoobVstWebguiFramework as useNoobVstWebguiFrameworkGeneric,
  useStream,
...
export function useNoobVstWebguiFramework() {
  const s = useNoobVstWebguiFrameworkGeneric();
  if (!wired) {
    wired = true;
...
export function bandCount() {
  return useNoobVstWebguiFrameworkGeneric().manifest.value?.meta?.bands || 24;
}
