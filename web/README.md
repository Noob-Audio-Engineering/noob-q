...
The user interface of **Noob-Q**, the free Pro-Q style equalizer by Noob Audio Engineering built on
...
over one loopback WebSocket using the `@noob-audio-engineering/noob-vst-webgui-framework` client library.

Everything specific to an equalizer lives here and in the Rust crate one
level up (this repository, see its [README](../README.md)). Everything generic, the client,
the reactive parameter handles, undo / redo, the knob, the spectrum and EQ
curve renderers, lives in the framework's browser package (see
[its README](https://github.com/Noob-Audio-Engineering/noob-vst-webgui-framework/blob/main/crates/noob-vst-webgui-framework/web/README.md)).
...
```sh
# from the repository root: serves web/dist on port 4242 (or the next free port)
cargo run --bin noob-q-standalone -- --open
```
...
cargo run --bin noob-q-standalone
# terminal 2: Vite on http://localhost:5173, proxying to the standalone
cd web && NOOB_VST_WEBGUI_FRAMEWORK_PORT=4242 npm run dev
```

The dev server proxies `/ws` (the WebSocket) and `/instance` + `/instances`
(the discovery endpoints used by the instance menu) to `NOOB_VST_WEBGUI_FRAMEWORK_PORT`. The
...
1. On load, `useNoobVstWebguiFramework()` creates one `NoobVstWebguiFrameworkClient` that connects to
...
└── (from @noob-audio-engineering/noob-vst-webgui-framework/vue) Knob, Popover, ContextMenu, LevelMeter
```
...
## The composable layer (`src/composables/useNoobVstWebguiFramework.js`)

| Export | Purpose |
|---|---|
| `useNoobVstWebguiFramework()` | Connection state (`ready`, `connected`, `manifest`, `stats`, `status`), `history`, plus Noob-Q's preset-modified tracking. Safe before `ready`. |
...
Handles need the manifest: call anything but `useNoobVstWebguiFramework()` only once
...
Vue components of `@noob-audio-engineering/noob-vst-webgui-framework` to Tailwind's scan; the `--noob-vst-webgui-framework-*`
