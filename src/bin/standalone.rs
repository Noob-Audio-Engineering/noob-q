...
//! the SPA from `web/dist` over noob-vst-webgui-framework. It exists so the page can be
...
//!
//! ```text
//! cargo run --bin noob-q-standalone -- [--port N] [--open] [--dir path]
//! ```
//!
...
//!   `npm run dev` with `NOOB_VST_WEBGUI_FRAMEWORK_PORT` set so Vite proxies `/ws` here.
...
//! Match references) persists in `<per-user data dir>/noob-vst-webgui-framework/noob-q.store.json`
//! through noob-vst-webgui-framework's `FileStore`; the plug-in keeps the same data inside its
//! host state instead.
//!
...
use noob_vst_webgui_framework::{AudioHandle, FileStore, NoobVstWebguiFramework, ServerConfig};
use serde_json::json;
...
fn apply_demo_preset(s: &NoobVstWebguiFramework) {
