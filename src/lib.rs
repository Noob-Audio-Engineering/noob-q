...
//! noob-vst-webgui-framework: the plug-in's editor is the operating system's web view,
...
//! everything reusable lives in the framework crates (`noob-vst-webgui-framework`,
//! `noob-vst-webgui-framework-nih`, `noob-vst-webgui-framework-webview`) and the browser library
//! (`@noob-audio-engineering/noob-vst-webgui-framework`).
//!
//! ## What the plug-in demonstrates
//!
...
//!   favourites, EQ Match references) through noob-vst-webgui-framework's UI store.
...
//! | Standalone | `src/bin/standalone.rs` | A fake audio thread on demo signals plus the noob-vst-webgui-framework server, for UI work without a DAW. |
//! | SPA | `web/` | The interface (Vue 3, Tailwind, Vite), built to `web/dist`. Documented in `crates/noob-vst-webgui-framework/web/README.md`. |
//!
...
//!
//! noob-vst-webgui-framework provides the bridge (parameters, streams, events, messages, the
